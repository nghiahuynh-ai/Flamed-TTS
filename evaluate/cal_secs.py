...
wav2mel = Wav2Mel()
dvector = torch.jit.load('/cm/archive/sonnn45/Dubbing/ckpts/dvector.pt').eval().to(device)
...
def eval(output_folder, gt_folder):
    ...
    with open(
            "/cm/archive/sonnn45/StyleDubber/dataset/GRID/Grid_Wav_22050_Abs_Feature/Refrence_list_self.txt", 
            "r",
             encoding="utf-8"
        ) as f:
            ...
            for line in tqdm(f.readlines()):
                ...
                output_file_path = os.path.join(output_folder, "{}.wav".format(name))
